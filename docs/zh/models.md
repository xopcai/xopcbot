...
xopcbot 通过 `~/.xopcbot/config.json` 中的 `config.providers` 支持自定义模型提供商。
...
| 方法 | 端点 | 说明 |
|------|------|------|
| GET | `/api/providers` | 获取自定义提供商配置 |
| POST | `/api/providers/validate` | 验证提供商配置 |
| PATCH | `/api/providers/:name` | 保存/更新提供商 |
| DELETE | `/api/providers/:name` | 删除提供商 |
| POST | `/api/providers/reload` | 热重载 |
| POST | `/api/providers/test-api-key` | 测试 API key 解析 |
...
2. 检查 `config.json` 文件是否正确保存
3. 如需可重启网关

### 从 models.json 迁移

如果您之前使用 `~/.xopcbot/models.json`，将内容移动到 `config.json` 的 `providers` 键下：

```bash
# 旧位置：~/.xopcbot/models.json
# 新位置：~/.xopcbot/config.json
```

只需将 `models.json` 中的 `providers` 对象复制到 `config.json` 即可。
