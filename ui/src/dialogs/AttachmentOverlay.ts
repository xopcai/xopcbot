...
import { base64ToArrayBuffer } from '../utils/attachment-utils';
...
export class AttachmentOverlay extends LitElement {
        ...
        <div class="bg-background/95 backdrop-blur border-b border-border" @click=${(e: Event) => e.stopPropagation()}>
          <div class="px-4 py-2 flex items-center justify-between">
            <div class="flex items-center gap-3 min-w-0">
              <span class="text-sm font-medium text-foreground truncate">${this.attachment.name}</span>
            </div>
            <div class="flex items-center gap-2">
              ...
              <button
                ...
                title="${i18n('Download')}"
              >
                ${unsafeHTML(iconToSvg(Download, 'w-4 h-4'))}
              </button>
              <button
                class="p-2 hover:bg-muted rounded-md transition-colors"
                @click=${() => this.close()}
                title="${i18n('Close')}"
              >
                ${unsafeHTML(iconToSvg(X, 'w-4 h-4'))}
              </button>
            </div>
          </div>
        </div>
  ...
  private renderToggle() {
    ...
    return html`
      <div class="flex items-center bg-muted rounded-md p-1">
        <button
          class="px-3 py-1 text-xs rounded ${!this.showExtractedText ? 'bg-background shadow-sm' : 'text-muted-foreground'}"
          @click=${() => { this.showExtractedText = false; this.error = null; }}
        >
          ${fileTypeLabel}
        </button>
        <button
          class="px-3 py-1 text-xs rounded ${this.showExtractedText ? 'bg-background shadow-sm' : 'text-muted-foreground'}"
          @click=${() => { this.showExtractedText = true; this.error = null; }}
        >
          ${i18n('Text')}
        </button>
      </div>
  ...
  private renderFileContent() {
    ...
    switch (fileType) {
      case 'image': {
        ...
        return html`
          <img src="${imageUrl}" class="max-w-full max-h-full object-contain rounded-lg shadow-lg" alt="${this.attachment.name}" />
        `;
      }
  ...
  private async renderPdf() {
    ...
    try {
      const arrayBuffer = base64ToArrayBuffer(this.attachment.content);
