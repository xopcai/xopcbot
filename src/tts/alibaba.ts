...
export class AlibabaProvider implements TTSProvider {
  ...
  constructor(config: AlibabaTTSConfig) {
    ...
    this.model = config.model || 'cosyvoice-v1';
  ...
  async speak(text: string, options?: TTSOptions): Promise<TTSResult> {
    ...
    try {
      log.debug({ model: this.model, voice: options?.voice || this.voice, textLength: limitedText.length }, 'Calling Alibaba CosyVoice');
      ...
      const data: CosyVoiceResponse = await response.json();
      
      // Get audio from base64
      let audioBuffer: Buffer;
      if (data.output.speech) {
