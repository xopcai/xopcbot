...
export function createOutboundSender(deps: OutboundSenderDeps) {
  ...
  async function sendTextMessage(
    ...
    chatId: string,
    content: string,
    options?: { threadId?: string; replyToMessageId?: string; silent?: boolean; accountId?: string }
  ): Promise<number> {
    // Get tableMode from account config
    const accountId = options?.accountId || 'default';
    const account = accountManager.getAccount(accountId);
    const tableMode = account?.tableMode as 'off' | 'bullets' | 'code' | undefined;
    
    const chunks = markdownToTelegramChunks(content, MESSAGE_CHUNK_SIZE, { tableMode });
