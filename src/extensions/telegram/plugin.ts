...
} from './telegram/adapters/index.js';
...
export type { TelegramResolvedAccount as TelegramAccount } from './telegram/adapters/index.js';
...
export class TelegramChannelPlugin implements ChannelPlugin<TelegramResolvedAccount> {
  ...
  readonly capabilities: ChannelCapabilities = TELEGRAM_REGISTRY_META.capabilities;
  ...
  readonly setupWizard: ChannelSetupWizard = {
    channel: 'telegram',
    status: {
      check: async (cfg, _accountId) => {
    ...
    finalize: {
      validate: async (cfg) => {
        ...
        const token =
          (accounts && Object.values(accounts).find((a) => a?.botToken)?.botToken) ||
          (tg?.botToken as string | undefined);
        return token ? { ok: true } : { ok: false, error: 'Configure channels.telegram with a bot token' };
      },
  ...
  gateway: ChannelGatewayAdapter<TelegramResolvedAccount> = {
    startAccount: async (ctx) => {
      await this.startAccount(ctx.account);
    },
    stopAccount: async (ctx) => {
      await this.accountManager.stopRunner(ctx.accountId);
    },
  };
