...
xopcbot 通过 `~/.xopcbot/models.json` 支持自定义模型提供商，类似于 [pi-coding-agent](https://github.com/mariozechner/pi-mono/tree/main/packages/coding-agent) 的做法。
...
- **默认**: `~/.xopcbot/models.json`
- **自定义**: 设置 `XOPCBOT_MODELS_JSON` 环境变量
...
- 描述性标题和说明
- **"添加您的第一个提供商"** 按钮
- 热门提供商快捷标签（Ollama、OpenRouter、LM Studio）

点击快捷标签会打开预填充配置的提供商对话框。
...
| 方法 | 端点 | 说明 |
|------|------|------|
| GET | `/api/models-json` | 获取当前配置 |
| POST | `/api/models-json/validate` | 验证配置 |
| PATCH | `/api/models-json` | 保存配置 |
