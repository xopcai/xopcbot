...
- **默认卡片**: 无阴影，主要靠 `border border-slate-100` 与灰色背景区分
...
- **侧边栏**: `dark:bg-slate-950` — 与全局背景同色，通过边框区分
...
- **边框**: `dark:border-slate-800` — 暗黑模式下主要靠**边框**和**背景色阶**区分层级
...
- **侧边导航**：固定宽度 240px，包含模块导航 + 用户信息，不可横向拖拽
