...
export function ThinkingDrawer({ blocks, onClose, isExiting = false }: ThinkingDrawerProps) {
  ...
  return (
    <aside
      className={cn(
        'thinking-drawer-panel flex max-h-full flex-col border-edge bg-surface-panel',
        isExiting && 'thinking-drawer-panel--exit',
        'max-md:fixed max-md:inset-x-0 max-md:bottom-0 max-md:z-30 max-md:max-h-[min(72vh,28rem)] max-md:rounded-t-xl max-md:border max-md:border-b-0 max-md:shadow-xl',
        'md:fixed md:inset-y-0 md:right-0 md:z-30 md:w-[var(--width-thinking-drawer)] md:border-l md:border-edge md:shadow-lg',
        'xl:relative xl:inset-auto xl:z-0 xl:h-full xl:w-[var(--width-thinking-drawer)] xl:shrink-0 xl:border-l xl:shadow-none',
      )}
    >
